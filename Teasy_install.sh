...
#: Origional Author: John
#: Origional Maintainers:
#: - Sypher
#: - nailshard

#: - Modified for takat.nl
#: - Version 0.2 
#: - Author Chrysolidocarpu
...
function generate_key_pair() {
  ...
  if [[ ! -e ${FTS_HOME}/.ssh/id_rsa.pub ]]; then
    # generate keys
    ssh-keygen -t rsa -f "${FTS_HOME}/.ssh/id_rsa" -N ""
  fi
...
function cleanup() {
  if [[ -n $NEEDRESTART ]]
  then
      cp $FTS_HOME/nr-conf-temp $NEEDRESTART
  fi
}
###############################################################################
